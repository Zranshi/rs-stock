...
def catch_symbol(page: int):
    ...
    """
    stock_api_url = (
        f"http://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/Market_Center.getHQNodeData"
...
async def chart_data(code: str):
    ...
    """
    res = session.query(SymbolDay).filter(SymbolDay.code == code).order_by(SymbolDay.day).all()
    data, time = [], []
    for item in res:
...
async def search_html(re: Request, up: bool, per: float, day: int):
    ...
    for item in data_now:
        if item.code in data_pre:
            rate = (
                (item.end_price - data_pre[item.code].end_price)
                / data_pre[item.code].end_price
                * 100
            )
            if rate > per if up else rate <= per:
                data.append({"now": item, "pre": data_pre[item.code], "rate": rate})
...
async def stock_info_html(re: Request, code: int):
    ...
    x = (
        session.query(SymbolDay.end_price, SymbolDay.day)
